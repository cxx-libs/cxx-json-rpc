...
private:
  Response call_method( const id_t& id, const std::string& name, const nlohmann::json& params ) const
  {
    ...
    try
    {
      ...
      if( !response.contains( "id" ) || !( response["id"].is_null() || response["id"].is_string() || response["id"].is_number_integer() ) )
        throw exception( internal_error, "The 'id' key is either missing or its type is invalid (expected 'null', 'string', 'integer')." );
      ...
      else if( response.contains( "error" ) && response["error"].is_string() )
        throw exception( internal_error, response["error"] );

      if( response.contains( "result" ) )
      {
        if( response["id"].is_string() ) return Response{ response["id"].get<std::string>(), response["result"].get<nlohmann::json>() };
        else
          return Response{ response["id"].get<int>(), response["result"].get<nlohmann::json>() };
      }
