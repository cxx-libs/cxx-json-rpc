...
public:
  ...
  int  dirty_method( int a, int b )
  {
    auto _ = std::to_string( a + b );
    throw std::exception();
  }
...
TEST_CASE_FIXTURE( Server, "invocations" )
{
  ...
  nlohmann::json params = nlohmann::json::parse( R"({"products": [{"id": 1, "price": 23.3, "name": "some product", "category": "cc"},{"id": 2, "price": 23.4, "name": "some product 2", "category": "order"}]})" );
